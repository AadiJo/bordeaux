...
(function () {
  ...
  function BigNum({ value, onChange, unit, step = 0.01, min, max, precision = 2 }) {
    const start = (down) => {
      down.preventDefault();
    ...
    return h('div', { className: 'rp-big', onPointerDown: (e) => { if (e.target.tagName !== 'INPUT') start(e); } },
      h('input', {
        value: typeof value === 'number' ? value.toFixed(precision) : value, inputMode: 'decimal',
  ...
  function RobotPage({ robot, setRobot }) {
    ...
    const rw = robot.w * unit, rl = robot.l * unit;
