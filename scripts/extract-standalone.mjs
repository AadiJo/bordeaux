...
.pathmgr-stats em {
  ...
  border-radius: 8px;
  color: #ffd28a;
  background: rgba(207,150,47,0.16);
  font-style: normal;
}
.plannerselect {
  display: inline-flex;
...
.plannerdiag {
  ...
  margin-top: 8px;
  padding: 8px 9px;
  border: 1px solid rgba(43,179,196,0.28);
  border-radius: 7px;
  background: rgba(43,179,196,0.08);
}
.plannerdiag b { color: #7bdcea; font-size: 12px; }
.plannerdiag span, .plannerdiag em { color: var(--muted); font-size: 11px; font-style: normal; }
.ctxinsp, .outline, .diag, .rt-panel, .stepinsp {
  border-color: var(--panel-border-strong) !important;
}
.ctxinsp-body, .outline-scroll { scrollbar-width: thin; }
.featempty, .diag-empty {
  color: color-mix(in srgb, var(--muted) 88%, white);
  background: rgba(255,255,255,0.032);
  border: 1px dashed rgba(255,255,255,0.11);
  border-radius: 6px;
}
.fieldlabel, .cgroup-h, .outline-grp {
...
legacyTemplate = legacyTemplate
  .split("Acquitaine")
  .join("Autonomous Routine")
  .replace(/<head[^>]*>/i, (match) => `${match}\n${resourceScript}\n${polishStyle}`);

fs.writeFileSync(path.join(legacyRoot, "index.html"), legacyTemplate);

console.log("Extracted PM math engine, field image, and legacy renderer");
