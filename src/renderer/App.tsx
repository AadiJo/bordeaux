    ...
    <div className="inspector-body">
      ...
      {selectedRange && selection?.kind === "range" && (
        <>
          ...
          <NumField label="Start" value={selectedRange.f0} min={0} max={1} step={0.01} onChange={(f0) => props.onPatchRange(selection.index, { f0 })} />
          <NumField label="End" value={selectedRange.f1} min={0} max={1} step={0.01} onChange={(f1) => props.onPatchRange(selection.index, { f1 })} />
          <NumField label="Max vel" value={selectedRange.maxVel} unit="m/s" step={0.1} onChange={(maxVel) => props.onPatchRange(selection.index, { maxVel })} />
        </>
      )}

      <div className="section-label">Global constraints</div>
      <NumField label="Max velocity" value={path.constraints.maxVel} unit="m/s" onChange={(maxVel) => props.onPatchPath({ constraints: { ...path.constraints, maxVel } })} />
      <NumField label="Max acceleration" value={path.constraints.maxAccel} unit="m/s²" onChange={(maxAccel) => props.onPatchPath({ constraints: { ...path.constraints, maxAccel } })} />
      <NumField label="Max deceleration" value={path.constraints.maxDecel} unit="m/s²" onChange={(maxDecel) => props.onPatchPath({ constraints: { ...path.constraints, maxDecel } })} />
    </div>
  );
}

function RobotPage(props: { project: BordeauxProject; onChange(robot: BordeauxProject["robot"]): void }) {
  const robot = props.project.robot;
  return (
    <section className="robot-page">
      <div className="robot-panel">
        <h3>Drivetrain</h3>
        <div className="drive-switch">
          <button className={robot.drive === "swerve" ? "active" : ""} onClick={() => props.onChange({ ...robot, drive: "swerve" })}>Swerve</button>
          <button className={robot.drive === "tank" ? "active" : ""} onClick={() => props.onChange({ ...robot, drive: "tank" })}>Tank</button>
        </div>
        <div className="two-col">
          <NumField label="Width" value={robot.w} unit="m" step={0.01} onChange={(w) => props.onChange({ ...robot, w })} />
          <NumField label="Length" value={robot.l} unit="m" step={0.01} onChange={(l) => props.onChange({ ...robot, l })} />
        </div>
        <NumField label="Max speed" value={robot.maxSpeed} unit="m/s" step={0.1} onChange={(maxSpeed) => props.onChange({ ...robot, maxSpeed })} />
      </div>
      <div className="optimization-panel">
        <h3>Optimization Track</h3>
        <div className="research-row"><ChevronRight size={16} /> profiledSpline is active for v1</div>
        <div className="research-row muted"><ChevronRight size={16} /> optimizedTrajectory is reserved for Sleipnir / Choreo-style sidecar work</div>
      </div>
    </section>
  );
}

function AutonomousPage({ project }: { project: BordeauxProject }) {
  return (
    <section className="auto-page">
      <div className="auto-panel">
        <h3>{project.routine?.name ?? "Autonomous Routine"}</h3>
        {(project.routine?.nodes ?? []).map((node) => (
          <div key={node.id} className="auto-step">
            <GitBranch size={16} />
            <span>{node.type === "path" ? project.paths[node.ref]?.name ?? "Missing path" : node.type === "decision" ? node.cond : node.title ?? node.cat}</span>
            <em>{node.type}</em>
          </div>
        ))}
      </div>
...
function ExportDialog(props: { preview: ReturnType<typeof previewBdxExport>; issues: ValidationIssue[]; onClose(): void; onExport(): void }) {
  return (
    <div className="modal-backdrop" role="presentation">
      <section className="modal" role="dialog" aria-modal="true" aria-labelledby="export-title">
        <header>
          <h3 id="export-title">Export .bdx</h3>
          <button onClick={props.onClose}>Close</button>
        </header>
        <div className="export-summary">
          <span><b>{props.preview.pathCount}</b> paths</span>
          <span><b>{props.preview.sampleCount}</b> samples</span>
          <span><b>{fmt(props.preview.totalTimeS)}</b>s total</span>
        </div>
        <div className="issues">
          {props.issues.length === 0 ? (
            <div className="issue ok"><Gauge size={16} /> Export is ready</div>
          ) : (
        ...
        <footer>
          ...
          <button className="primary" disabled={!props.preview.ok} onClick={props.onExport}><Download size={16} />Export</button>
        </footer>
      </section>
    </div>
  );
